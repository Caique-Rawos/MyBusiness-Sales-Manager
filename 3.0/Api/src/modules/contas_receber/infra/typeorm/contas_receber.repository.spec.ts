import { ContasReceberTypeOrmRepository } from './contas_receber.repository';

describe('ContasReceberTypeOrmRepository', () => {
  let typeOrmRepository: any;
  let repository: ContasReceberTypeOrmRepository;
  ...
  beforeEach(() => {
    ...
    repository = new ContasReceberTypeOrmRepository(tenantContext);
  });
  ...
  it('should update only valorTotal', async () => {
    typeOrmRepository.update.mockResolvedValue(undefined);
    await repository.updateValorTotal(1, 150);
    expect(typeOrmRepository.update).toHaveBeenCalledWith(1, { valorTotal: 150 });
  });
